...
#ifdef ESP32
#if defined(ESP_ARDUINO_VERSION) && defined(ESP_ARDUINO_VERSION_VAL) && (ESP_ARDUINO_VERSION >= ESP_ARDUINO_VERSION_VAL(2, 0, 0))
void WiFiManager::WiFiEvent(arduino_event_id_t event, arduino_event_info_t info){
    ...
    if(event == ARDUINO_EVENT_WIFI_STA_DISCONNECTED){
      ...
      DEBUG_WM(DEBUG_VERBOSE,"[EVENT] WIFI_REASON:",disconnected.reason);
      if(disconnected.reason == WIFI_REASON_AUTH_EXPIRE || disconnected.reason == WIFI_REASON_AUTH_FAIL){
      ...
      if(disconnected.reason == WIFI_REASON_NO_AP_FOUND){
        ...
        DEBUG_WM(DEBUG_VERBOSE,"[Event] ARDUINO_EVENT_WIFI_STA_DISCONNECTED, reconnecting");
        WiFi.reconnect();
      #endif
    }
