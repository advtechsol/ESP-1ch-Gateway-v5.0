...
#include "GatewayBase64.h"						// https://github.com/adamvr/arduino-base64 (changed the name)
...
#	if defined(ESP32_ARCH) && (_BOARD_PROFILE == BOARD_PROFILE_HELTEC_WIFI_LORA)
	Heltec.begin(false /* Display */, true /* LoRa */, true /* Serial */, true /* PABOOST */, 868E6);
#	endif
...
#	if defined(ESP32_ARCH)
	...
	if (!spiffsOk) {
		spiffsOk = SPIFFS.begin(true);					// Final attempt with auto-format
	}
#	else
...
#		endif //_MONITOR
		...
		msg_oLED("NO FS");
		while (true) {
			delay(1000);										// Halt; nothing works without SPIFFS
		}
	}
...
#		endif //_MONITOR
		...
		// If we are still not connected after WiFiManager, fall back to AP mode
